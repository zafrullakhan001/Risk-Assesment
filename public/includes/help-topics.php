...
<p>Under <strong>Original files</strong>, select one or more files and use <strong>Reparse selected</strong> to refresh Demand, Story, Task, related-ticket, or DDR information. Ticket PDFs exported by console sync are also parsed automatically when received. Use <strong>Delete selected</strong> to remove files you do not need; already-parsed dossier information is retained. Extra copies with the same file name and size can be <strong>hidden</strong> in the list or <strong>removed</strong> so ZIP exports stay smaller (one copy of each is kept).</p>
