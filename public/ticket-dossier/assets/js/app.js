(function () {
    ...
    var themeKey = 'ra-theme';
    var sizeKey = 'ra-size';
    var allowedSizes = ['auto', 's', 'm', 'l', 'xl', 'xxl'];
    var root = document.documentElement;
    var maxFiles = 10;

    function getTheme() {
        return localStorage.getItem(themeKey) || 'teal';
    }

    function getSizePref() {
        var pref = localStorage.getItem(sizeKey) || 'auto';
        return allowedSizes.indexOf(pref) !== -1 ? pref : 'auto';
    }

    function autoSize() {
        var width = window.innerWidth;
        if (width < 1280) return 's';
        if (width < 1540) return 'm';
        if (width < 1800) return 'l';
        if (width < 2200) return 'xl';
        return 'xxl';
    }
    ...
    function syncThemeButtons() {
        ...
        document.querySelectorAll('[data-theme-set]').forEach(function (button) {
            var isActive = button.getAttribute('data-theme-set') === activeTheme;
    ...
    function syncSizeButtons() {
        var activePref = getSizePref();
        document.querySelectorAll('[data-size-set]').forEach(function (button) {
            var isActive = button.getAttribute('data-size-set') === activePref;
            button.classList.toggle('is-active', isActive);
            button.setAttribute('aria-pressed', isActive ? 'true' : 'false');
        });
    }
    ...
    function setTheme(theme) {
        var nextTheme = theme === 'indigo' ? 'indigo' : 'teal';
    ...
    function applySize(pref) {
        var nextPref = allowedSizes.indexOf(pref) !== -1 ? pref : 'auto';
        var size = nextPref === 'auto' ? autoSize() : nextPref;
        try { localStorage.setItem(sizeKey, nextPref); } catch (e) {}
